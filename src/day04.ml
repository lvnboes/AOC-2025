let map_warehouse_line (line : string) =
  ...
  |> Seq.map (function
    | '.' -> 0
    | '@' -> 1
    | _ -> invalid_arg "warehouse line")
  |> List.of_seq

let padd_map (lst : 'a list) (padding : 'a) (f : 'a -> 'a) : 'a list =
...
let padd_matrix (matrix : int list list) =
  ...
  padd_map matrix matrix_padding (fun row -> padd_map row 0 (fun x -> x))

let clean_row (before : int list) (row : int list) (next : int list)
    (count : int) : int list * int =
  let rec aux acc count before row next =
    match (before, row, next) with
    | ( a :: (b :: c :: _ as t1),
        d :: (e :: f :: _ as t2),
        g :: (h :: i :: _ as t3) ) ->
